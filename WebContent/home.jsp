		...
		<script>
			function scroll_follow( id )
			{
			  ...
			  { 
				  ...
				  $( id ).stop().animate({top:position+120+"px"}, 0); //해당 오브젝트 위치값 재설정
			   });
			}
			...
			$(function(){
				$writePostBtn = $('button.btn.btn-small.btn-danger.writeBtn');
			...
			$(function(){
				...
				$secretBtn.click(function(e){
					...
					$secretBtn.attr('src','image/selectedSecret.png');
					$allBtn = $('img.allBtn');
					$allBtn.attr('src','image/all.png');
					$friendBtn = $('img.friendBtn');
					$friendBtn.attr('src','image/friend.png');
								...
								<tr>
									<c:choose>
										<c:when test="${postAllInfoBean.postingViewBean.mypageTitle!=null}">
											<td  rowspan="2" class="profileTd"><a href="page_main.do?title=${postAllInfoBean.postingViewBean.mypageTitle}"><img src="${postAllInfoBean.postingViewBean.mypageImg}" class="profile"></a></td>
											<td class="profileTd"><a href="page_main.do?title=${postAllInfoBean.postingViewBean.mypageTitle}" id="nameA">${ postAllInfoBean.postingViewBean.mypageTitle }</a></td>
										</c:when>
										...
										<ul class="sub-wrapper postUl">
											<c:choose>
											  ...
											  <c:otherwise>
													...
													<li><a href="blockUser?userId=${ postAllInfoBean.postingViewBean.id}">작성자차단</a></li>
											  </c:otherwise>
											</c:choose>
										</ul>
									</td>
